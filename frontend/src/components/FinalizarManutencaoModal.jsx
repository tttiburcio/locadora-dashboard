...
import { dbFinalizarManutencao } from '../utils/api'
...
export default function FinalizarManutencaoModal({ manutencao, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await dbFinalizarManutencao(manutencao.id, payload)
      onSaved()
    } catch (err) {
      ...
      <div className="bg-g-900 border border-g-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-fade-up">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-g-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-g-200 font-semibold text-sm">Finalizar OS</h2>
              <p className="text-g-600 text-xs font-mono">{manutencao.placa} — {manutencao.servico || manutencao.sistema || '—'}</p>
            </div>
          </div>
