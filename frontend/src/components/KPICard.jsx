    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className={`p-1.5 rounded-md
          ...
          ${accent  ? 'bg-white/5'     : ''}
          ${!danger && !accent ? 'bg-g-800' : ''}
        `}>
          {Icon && (
            <Icon className={`w-4 h-4
              ${danger ? 'text-red-400' : accent ? 'text-g-300' : 'text-g-500'}
            `} />
          )}
        </div>
        {trend !== undefined && (
      ...
      {/* Value */}
      <div>
        <p className="text-g-500 text-xs font-semibold uppercase tracking-wider mb-1">{label}</p>
        <p className={`font-bold text-xl leading-tight tabular-nums
          ${danger ? 'text-red-300' : 'text-g-50'}
        `}>
          {displayed}
        </p>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-px
        ${danger ? 'bg-red-700/50' : accent ? 'bg-white/20' : 'bg-g-800'}
      `} />
    </div>
  )
}
